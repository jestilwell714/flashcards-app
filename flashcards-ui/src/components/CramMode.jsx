...
export default function CramMode({ item }) {
  ...
  const fetchMoreCardsUrl =
    type === "root"
      ? `${API_BASE_URL}/api/cram`
      : `${API_BASE_URL}/api/cram/${type}/${id}`;
  ...
  if (cards.length === 0 || !card) {
    if (isFinished) {
      ...
      return (
        <div className="text-center flex flex-col content-center h-dvh w-screen py-32 px-4">
          <p className="text-white mt-4 font-bold uppercase tracking-widest text-xl">
            This directory contains no FlashCards
          </p>
          <p className="text-white mt-4 font-bold uppercase tracking-widest text-xl">
            Please add FlashCards
          </p>
          <LoadingSpinner />
          <p className="text-white font-medium">You are being redirected</p>
        </div>
      );
    } else {
      fetchMoreCards();
  ...
  return (
    <div className="h-dvh w-screen flex flex-col content-between items-center justify-between gap-4">
      <nav className="flex flex-row items-center justify-between w-full p-4">
        ...
        {type != "root" && (
          <h2 className=" mx-5 text-3xl text-bold truncate text-white">
            {item.name}
          </h2>
        )}
      ...
      <div
        className={`transition-opacity duration-300 ${isFlipped ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
