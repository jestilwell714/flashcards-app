...
export default function CreateDeckFolderOrTag({
  parentId,
  type,
  initialData,
  onSubmit,
  ...
  const cramModeUrl = isEdit
    ? type === "root"
      ? "/explorer/cram/root/0"
      : `/explorer/cram/${type}/${initialData.id}`
    : ``;
  ...
  function handleSubmit(e) {
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok)
          console.error("Database didn't create/edit deck, folder or tag");
  ...
  return (
    <div
      className={
        isEdit
          ? `mx-6 mt-4 flex-1 flex flex-col items-center justify-center h-full gap-10`
          : ``
      }
    >
      ...
      {isEdit && (
        <button
          ...
          onClick={() => handlePlay(cramModeUrl)}
        >
          Play
        </button>
      )}
